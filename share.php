...
require_once __DIR__ . '/auth.php'; // for get_db()

// Start session minimally (no auth required)
...
if ($token && preg_match('/^[a-f0-9]{48}$/', $token)) {
    $db   = get_db();
    $stmt = $db->prepare('SELECT * FROM images WHERE share_token = ?');
    $stmt->execute([$token]);
    $image = $stmt->fetch();
    if ($image) {
        $ts = $db->prepare(
...
<title><?= $image ? htmlspecialchars($image['title'] ?: $image['original_name']) . ' — Alea Lab Vault' : 'Not Found — Alea Lab Vault' ?></title>
...
<div class="wrap">
  <div class="topbar">
    ...
    <span class="badge">Shared Image</span>
  </div>
  ...
  <?php if ($image): ?>
    ...
    <div class="img-title"><?= htmlspecialchars($image['title'] ?: $image['original_name']) ?></div>
    <div class="img-meta">
      <?= date('F j, Y', strtotime($image['uploaded_at'])) ?> ·
    ...
    <?php if ($tags): ?>
      <div class="tags">
        <?php foreach ($tags as $tag): ?>
          <span class="tag" style="color:<?= htmlspecialchars($tag['color']) ?>;border-color:<?= htmlspecialchars($tag['color']) ?>33">
    ...
    <?php if ($image['notes']): ?>
      <div class="notes"><?= nl2br(htmlspecialchars($image['notes'])) ?></div>
    <?php endif; ?>

    <a href="uploads/<?= htmlspecialchars($image['filename']) ?>" download="<?= htmlspecialchars($image['original_name']) ?>" class="btn-dl">
      ↓ Download
    </a>

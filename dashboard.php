...
foreach ($images as &$img) {
    $ts = $db->prepare('SELECT t.* FROM tags t JOIN image_tags it ON it.tag_id=t.id WHERE it.image_id=?');
    $ts->execute([$img['id']]);
...
<div class="app">
  ...
  <main class="main">
    ...
    <div class="stats">
      <div class="stat"><div class="stat-label">Images</div><div class="stat-val"><?= count($images) ?></div></div>
    ...
    <div class="tag-filters" id="tagFilters">
      ...
      <?php foreach ($allTags as $tag): ?>
        <button class="tag-pill" data-tag-id="<?= $tag['id'] ?>" style="color:<?= htmlspecialchars($tag['color']) ?>">
    ...
    <div class="gallery" id="gallery">
      <?php if (empty($images)): ?>
        <div class="empty"><span class="icon">⬡</span>No images yet — tap ＋ to upload.</div>
      <?php else: ?>
        <?php foreach ($images as $i => $img): ?>
          <div class="img-card" style="animation-delay:<?= $i*.03 ?>s"
               data-id="<?= $img['id'] ?>"
               data-title="<?= htmlspecialchars(addslashes($img['title'] ?? '')) ?>"
               data-orig="<?= htmlspecialchars(addslashes($img['original_name'])) ?>"
            ...
            <img class="img-thumb" src="uploads/<?= htmlspecialchars($img['filename']) ?>"
                 alt="<?= htmlspecialchars($img['original_name']) ?>" loading="lazy">
            <div class="img-body">
              <div class="img-title-sm"><?= htmlspecialchars($img['title'] ?: $img['original_name']) ?></div>
              <?php if ($img['tags']): ?>
                <div class="img-tags">
                  <?php foreach ($img['tags'] as $t): ?>
                    <span class="tag-chip" style="color:<?= htmlspecialchars($t['color']) ?>;border-color:<?= htmlspecialchars($t['color']) ?>44">
        ...
        <div class="tag-select" id="uploadTagSelect">
          <?php foreach ($allTags as $tag): ?>
            <button type="button" class="tag-toggle" data-id="<?= $tag['id'] ?>"
                    ...
                    onclick="toggleUploadTag(this)"><?= htmlspecialchars($tag['name']) ?></button>
          <?php endforeach; ?>
        </div>
      </div>
    ...
    <div id="tagsList">
      <?php foreach ($allTags as $tag): ?>
        <div class="tag-mgmt-row" id="tagRow_<?= $tag['id'] ?>">
          <div class="tname">
            <span class="swatch" style="background:<?= htmlspecialchars($tag['color']) ?>"></span>
    ...
    <div style="margin-top:1.5rem">
      ...
      <div class="field">
        ...
        <div class="color-swatches" id="colorPicker">
          <?php foreach (['#00e5ff','#ffe066','#a78bfa','#34d399','#ff3d71','#fb923c','#f472b6','#60a5fa'] as $c): ?>
...
let allTagsData = <?= json_encode(array_values($allTags)) ?>;
...
function openDetail(card){
  ...
  document.getElementById('detailDeleteId').value = activeImage.id;
  const dl = document.getElementById('detailDownload');
  dl.href = activeImage.src; dl.download = activeImage.orig;
...
async function saveImageEdits(){
  ...
  await api('update_image',{image_id:activeImage.id,title:activeImage.title,notes,csrf_token:CSRF});
  await api('set_image_tags',{image_id:activeImage.id,tag_ids:JSON.stringify([...selectedDetailTags]),csrf_token:CSRF});
  ...
  if(card){
    ...
    card.querySelector('.img-tags').innerHTML = selTags.map(t=>
      `<span class="tag-chip" style="color:${t.color};border-color:${t.color}44">${t.name}</span>`
    ).join('');
  }
}
...
async function genShareLink(){
  const res = await api('get_share_token',{image_id:activeImage.id,csrf_token:CSRF});
  if(res.ok){
    const url=`${BASE}/share.php?token=${res.token}`;
...
async function revokeShare(){
  ...
  const res = await api('revoke_share',{image_id:activeImage.id,csrf_token:CSRF});
...
async function createTag(){
  ...
  if(!name) return;
  const res=await api('create_tag',{name,color:selectedTagColor,csrf_token:CSRF});
  if(res.ok){
...
async function deleteTag(id){
  ...
  const res=await api('delete_tag',{tag_id:id,csrf_token:CSRF});
  if(res.ok){
...
async function api(action,data){
  ...
  try { const r=await fetch('api.php',{method:'POST',body:form}); return await r.json(); }
